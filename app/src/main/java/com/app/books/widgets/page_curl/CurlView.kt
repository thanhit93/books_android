package com.app.books.widgets.page_curl
 ...
 * @author harism
 */
class CurlView : GLSurfaceView, OnTouchListener, CurlRenderer.Observer {
    ...
    override fun onDrawFrame() {
        ...
        if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
            if (mAnimationTargetEvent == SET_CURL_TO_RIGHT) {
                ...
                val right: CurlMesh = mPageCurl
                val curl: CurlMesh = mPageRight
                ...
                val left: CurlMesh = mPageCurl
                val curl: CurlMesh = mPageLeft
    ...
    override fun onTouch(view: View, me: MotionEvent): Boolean {
        ...
        val rightRect: RectF = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT)!!
        val leftRect: RectF = mRenderer.getPageRect(CurlRenderer.PAGE_LEFT)!!
    ...
    private fun setCurlPos(curlPos: PointF, curlDir: PointF, radius: Double) {
        ...
        if (mCurlState == CURL_RIGHT
            ...
            val pageRect: RectF = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT)!!
            ...
            val pageRect: RectF = mRenderer.getPageRect(CurlRenderer.PAGE_LEFT)!!
            if (curlPos.x <= pageRect.left) {
                mPageCurl.reset()
                requestRender()
                return
    ...
    private fun updateCurlPos(pointerPos: PointerPosition) {
        ...
        // Default curl radius.
        var radius: Float = mRenderer.getPageRect(CURL_RIGHT)!!.width() / 3
        ...
        if (mCurlState == CURL_RIGHT
            ...
            val pageWidth: Float = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT)!!.width()
            var curlLen = radius * Math.PI
            if (dist > pageWidth * 2 - curlLen) {
                curlLen = max(pageWidth * 2 - dist, 0f).toDouble()
                radius = (curlLen / Math.PI).toFloat()
            }
            ...
            if (dist >= curlLen) {
                ...
                if (mViewMode == SHOW_TWO_PAGES) {
                    mCurlPos.x -= mCurlDir.x * (translate.toFloat()) / dist
                } else {
                    val pageLeftX: Float = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT)!!.left
                    radius = max(min(mCurlPos.x - pageLeftX, radius), 0.0f)
                }
                mCurlPos.y -= mCurlDir.y * (translate.toFloat()) / dist
            } else {
            ...
            val pageLeftX: Float = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT)!!.left
            radius = max(min(mCurlPos.x - pageLeftX, radius), 0.0f)
            val pageRightX: Float = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT)!!.right
            mCurlPos.x -= min(pageRightX - mCurlPos.x, radius)
            mCurlDir.x = mCurlPos.x + mDragStartPos.x
            mCurlDir.y = mCurlPos.y - mDragStartPos.y
        }
        setCurlPos(mCurlPos, mCurlDir, radius.toDouble())
    }
